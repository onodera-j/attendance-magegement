...
@section('content')
    <div class="content">
        ...
        <div class="select-month">
            <div class="last-day">
                <a class="link-date"
                    href="{{ route('admin.attendance.list', ['year' => $viewDates['prevYear'], 'month' => $viewDates['prevMonth'], 'day' => $viewDates['prevDay']]) }}">前日</a>
            </div>
            <div class="this-day">
                <img class="icon-calender" src="{{ asset('img/calender.svg') }}"><span
                    class="date">{{ $viewDates['titleDate']->format('Y/m/d') }}</span>
            </div>
            <div class="next-day">
                <a class="link-date"
                    href="{{ route('admin.attendance.list', ['year' => $viewDates['nextYear'], 'month' => $viewDates['nextMonth'], 'day' => $viewDates['nextDay']]) }}">翌日</a>
            </div>
        </div>
                ...
                @foreach ($attendanceDatas as $attendanceData)
                    <tr class="table-row">
                        ...
                        <td class="table-data td-detail">
                            <a class="link-detail" href="/attendance/{{ $attendanceData->id }}">詳細</a>
                        </td>
                    </tr>
                @endforeach
                <tr class="table-row">
